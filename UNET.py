...
@register_keras_serializable(package="Custom", name="take_last")
def take_last(x):
    # x: (B, T, H, W, C) -> (B, H, W, C), last timestep
    return x[:, -1]
...
spatial_padded = np.pad(
    spatial_features,
    ((0, 0), (top_pad, pad_height - top_pad), (left_pad, pad_width - left_pad), (0, 0)),
    mode='reflect'
)
...
def build_bottleneck_convlstm_unet_model(hp):
    with strategy.scope():
        ...
        skip1 = Lambda(take_last, name="take_last_c1")(c1_seq)
        skip2 = Lambda(take_last, name="take_last_c2")(c2_seq)
        skip3 = Lambda(take_last, name="take_last_c3")(c3_seq)
...
def visualize_trial(trial_id):
    ...
    model = tf.keras.models.load_model(model_fp, custom_objects=custom_objects, safe_mode=False)
...
class MyBayesianTuner(kt.BayesianOptimization):
    def run_trial(self, trial, *args, **kwargs):
        ...
        try:
            ...
            logging.warning(f"Visualization failed for trial {trial.trial_id}: {e}")
